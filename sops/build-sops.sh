...
{ 
    ...
    pandoc -s -f gfm -t html5 -o $PKG/usr/doc/${pkgname}-$pkgver/README.html $startdir/README.md;
    rm $PKG/usr/share/SOPS/{LICENCE,README.md,TODO};
    pandoc -s -f dokuwiki -t html5 -o $PKG/usr/doc/${pkgname}-$pkgver/usage.html $startdir/user.txt;
...
sops: sops (simple plugin system for orca with plugins)
sops: 
sops: 
sops: 
sops: 
