...
function gen_packages_txt {
	echo '' > PACKAGES.TXT
	find ./packages -type f -name '*.meta' -exec cat {} \; >> PACKAGES.TXT
...
case "$1" in
	...
	;;
	all)
		for pkg in `find ./packages -type f -name '*.t[glx]z' -print`
		do
			gen_meta $pkg
		done
		gen_packages_txt
	...
	md5)
		...
		gpg2 -sba CHECKSUMS.md5.gz
	;;
	clean)
		find ./packages -type f -name "*.meta" -exec rm '{}' \;
		for pkgtxt in $(find ./packages -type f -name "*.txt") ; do
			sed "s/[^:]\{1,\}://" $pkgtxt > ${pkgtxt%txt}desc
			rm -f $pkgtxt
		done
	;;
	deps)
		rm -f /tmp/depserrors.txt
		echo > /tmp/depserrors.txt
		( cd ./packages
		for pkg in $(ls *.t?z); do
			printf "%b" "$(basename $pkg)\n"
			printf "%b" "$(basename $pkg)\n" >>/tmp/depserrors.txt
			DEPFINDERNOEXIT=1 depfinder -f -j4 $pkg 2>>/tmp/depserrors.txt
		done
		printf "%b" "Check /tmp/depserrors.txt for errors\n"
		)
	;;
	*)
		echo "$0 [pkg [file]|all|new|PACKAGESTXT|md5|clean]"
	;;
esac
