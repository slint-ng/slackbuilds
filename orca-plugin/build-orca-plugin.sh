...
{ 
    ...
    git checkout $commit;
    autoreconf -vif;
    ./configure --prefix=/usr --libdir=/usr/lib${LIBDIRSUFFIX} --sysconfdir=/etc --localstatedir=/var --mandir=/usr/man --docdir=/usr/doc/$pkgname-$pkgver --build=$arch-slackware-linux;
    make;
...
orca-plugin: orca (screen reader)
orca-plugin: 
orca-plugin: Orca is a free, open source, flexible, and extensible screen reader.
orca-plugin: The xdesktop version is to be used in enviromments other than Gnome3.
orca-plugin: 
orca-plugin: Homepage: http://live.gnome.org/Orca/
orca-plugin: 
orca-plugin: 
orca-plugin: 
orca-plugin: 
