...
txz=$(find  "$REPO"/packages -name "${pkgname}-*txz"|grep "${pkgname}-[^-]*-[^-]*-[^-]*$")
md5=$(find  "$REPO"/packages -name "${pkgname}-*md5"|grep "${pkgname}-[^-]*-[^-]*-[^-]*$")
txt=$(find  "$REPO"/packages -name "${pkgname}-*txt"|grep "${pkgname}-[^-]*-[^-]*-[^-]*$")
meta=$(find  "$REPO"/packages -name "${pkgname}-*meta"|grep "${pkgname}-[^-]*-[^-]*-[^-]*$")
dep=$(find  "$REPO"/packages -name "${pkgname}-*dep"|grep "${pkgname}-[^-]*-[^-]*-[^-]*$")
con=$(find  "$REPO"/packages -name "${pkgname}-*con"|grep "${pkgname}-[^-]*-[^-]*-[^-]*$")
# echo txz=$txz
# echo md5=$md5
# echo txt=$txt
# echo meta=$meta
...
if [ ! "$pkgname" = "" ]; then
	...
	printf '%s' "Include $packagefullname in Packages/Testing/Installer/None [p/t/i/n] "
	read -r dummy
	if [ "$dummy" = "p" ] || [ "$dummy" = "P" ]; then
		mv "$packagefullname" "$REPO/packages" || exit 1
	elif [ "$dummy" = "t" ] || [ "$dummy" = "T" ]; then
		mv "$packagefullname" "$REPO/packages/testing" || exit 1
	elif [ "$dummy" = "i" ] || [ "$dummy" = "I" ]; then
		mv "$packagefullname" "$REPO/packages/installer" || exit 1
	else
		echo "non inclus."
	fi
	packagedep="$(find . -name "${pkgname}*dep")"
	printf '%s' "Include $packagedep in Packages/Testing/None [p/t/i/n] "
	read -r dummy
	if [ "$dummy" = "p" ] || [ "$dummy" = "P" ]; then
		mv "$packagedep" "$REPO/packages" || exit 1
	elif [ "$dummy" = "t" ] || [ "$dummy" = "T" ]; then
		mv "$packagedep" "$REPO/packages/testing" || exit 1
	elif [ "$dummy" = "i" ] || [ "$dummy" = "I" ]; then
		mv "$packagedep" "$REPO/packages/ISO" || exit 1
	else
		echo "not included."
	fi
fi
