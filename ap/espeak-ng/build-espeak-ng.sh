...
source=("README.mbrola" "list_of_mbrola_voices" "espeak-ng-1.52.0.tar.gz")
...
{ 
    ...
    ronn -m *ronn );
    cmake -B build -S "${pkgname}-$pkgver" -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE='None' -DFETCHCONTENT_FULLY_DISCONNECTED=ON -DBUILD_SHARED_LIBS=ON -DESPEAK_COMPAT=ON -DESPEAK_BUILD_MANPAGES=OFF -DUSE_KLATT=OFF -DUSE_MBROLA=OFF -Wno-dev;
    cmake --build build;
...
espeak-ng: espeak-ng (Next Generation Text-to-Speech program)
espeak-ng: 
espeak-ng: open source speech synthesizer supporting 100 languages and accents.
espeak-ng: It is based on the eSpeak engine created by Jonathan Duddington. It
espeak-ng: uses spectral formant synthesis by default which sounds robotic, but
espeak-ng: can be configured to use Klatt formant synthesis or MBROLA to give it
espeak-ng: a more natural sound.
espeak-ng: 
espeak-ng: HomePage:https://github.com/rhdunn/espeak/
espeak-ng: 
espeak-ng: 
EODESC
}
